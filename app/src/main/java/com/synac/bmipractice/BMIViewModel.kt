...
class BMIViewModel : ViewModel() {
    ...
    fun onAction(action: BMIActions) {
        when (action) {
            ...
            is BMIActions.SheetItemClicked -> changeWeightOrHeightUnit(action.sheetItem)
            is BMIActions.Number -> enterNumber(action.number)
            BMIActions.AllClear -> allClearToZero()
            BMIActions.Delete -> deleteLastDigit()
            BMIActions.GoButton -> calculateBMI()
        }
    }
